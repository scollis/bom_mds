subroutine gracon_vel2d_3d(gv_u, gv_v, F, u_array, v_array, i_cmpt_r1, j_cmpt_r1, i_cmpt_r2,&
                           j_cmpt_r2, nx, ny, nz, vr1, vr2, weights)
implicit none
!works on a cube of data but only get v and u
...
!----------------------COST FUNCTION--------------------
            do i=1,nx
               ...
               enddo
            enddo
            call vel_2d_3d_cost(gv_u, gv_v, F, u_array, v_array, i_cmpt_r1, j_cmpt_r1, i_cmpt_r2,&
                                j_cmpt_r2, nx, ny,nz, vr1, vr2, weights)
            !unpack variables
            gradi=0.0
            do i=1,nx
...
do i=1,nx
   do j=1,ny
      do k=1,nz
         ...
         u_array(i,j,k)=X(i+nx*(j-1)+nx*ny*(k-1))
         v_array(i,j,k)=X(i+nx*(j-1) + nx*ny*(k-1) + nx*ny*nz)
         gv_u(i,j,k)=G(i+nx*(j-1)+nx*ny*(k-1))
         gv_v(i,j,k)=G(i+nx*(j-1) + nx*ny*(k-1) + nx*ny*nz)
      enddo
   enddo
enddo
call rms2(iter-1,F,gradi)
