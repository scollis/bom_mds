subroutine vel_2d_cost(gv_u, gv_v, cost, u_array, v_array, i_cmpt_r1, j_cmpt_r1, i_cmpt_r2, j_cmpt_r2, nx, ny, vr1, vr2, weights)
implicit none
...
real(kind=8), dimension(nx,ny):: i_cmpt_r1, j_cmpt_r1, i_cmpt_r2, j_cmpt_r2, weights
...
!Python bindings
!f2py intent(in) gv_u, gv_v, cost, u_array, v_array, i_cmpt_r1, j_cmpt_r1, i_cmpt_r2, j_cmpt_r2, nx, ny, vr1, vr2, weights
...
do i=1,nx
  do j=1,ny
    ...
    cost=cost+0.5*weights(i,j)*((vr1(i,j)-vr1_sim)**2 + (vr2(i,j)-vr2_sim)**2)
    ...
    gv_u(i,j)=gv_u(i,j) + weights(i,j)*(i_cmpt_r1(i,j)*(vr1_sim-vr1(i,j)) + i_cmpt_r2(i,j)*(vr2_sim-vr2(i,j)))
    ...
    gv_v(i,j)=gv_v(i,j)+ weights(i,j)*(j_cmpt_r1(i,j)*(vr1_sim-vr1(i,j)) + j_cmpt_r2(i,j)*(vr2_sim-vr2(i,j)))
   enddo
enddo
!print*,'cost is ', cost 
return
